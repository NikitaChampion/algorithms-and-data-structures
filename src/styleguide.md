...
1. `#include <bits/stdc++.h>` использовать нельзя.<details><summary>Подробнее</summary>
...
3. Не подключайте C-шные библиотеки с помощью `#include <название_библиотеки.h>`.<details><summary>Пример</summary>
    Пишите не так:
...
4. `using namespace std;` использовать нельзя.<details><summary>Подробнее</summary>
...
7. Не используйте ввод-вывод в стиле С через функции scanf, printf. <details><summary>Подробнее</summary>
...
9. Не используйте тип `long`. <details><summary>Подробнее</summary>
...
12. Вставляйте слово `const` везде, где только это возможно по смыслу. <details><summary>Подробнее</summary>
...
13. Используйте везде в программе индексацию с нуля. <details><summary>Подробнее</summary>
...
15. Не вычитайте никогда просто так ничего из `container.size()`, где `container` &mdash;  какой-нибудь контейнер из STL. <details><summary> Подробнее </summary>
...
19. Старайтесь не использовать по возможности динамическое выделение памяти (с помощью `new` и `malloc`): <details><summary>Почему</summary>
...
20. При использовании `vector` имейте в виду, что у него есть удобные методы: различные конструкторы, позволяющие задать размер и значение элемента вектора по умолчанию, операторы присваивания, сравнения (лексикографического) и оператор `swap`. <details><summary>Примеры:</summary>
...
23. В большинстве случаев нельзя сравнивать числа типа `float` и `double` просто операторами <, >, <=, >=, ==: <details><summary>Почему</summary>
...
24. Для своих типов (классов, структур), если объекты типа необходимо сравнивать между собой, реализуйте всегда `operator<` и не реализуйте остальные операторы сравнения (`operator<=, operator>, operator>=`): <details><summary>Почему</summary>
...
25. Не используйте `std::pair` (за исключением случая, описанного ниже). <details><summary>Подробнее</summary>
